<template>
    <div class="flex-col xl:w-7/12 p-[40px]">
        <div class="flex-col relative">
            ...
            <button :class="[previewStore.titlePreview === '' ? 'hidden' : '']" @click="copyTitle()" class="bg-white/50 rounded p-1 absolute bottom-7 right-2 w-[22px] h-[24px]">
        ...
        <div class="flex-col mt-8 relative">
            ...
            <textarea class="border-white text-sm leading-5 w-full py-2 px-3 border-2 min-h-[10em] text-white rounded-lg shadow-sm focus:outline-none focus:ring focus:border-white focus:ring-blue-500 bg-transparent placeholder:italic placeholder:text-slate-100" id="description" name="description" type="text" placeholder="The description of your website, be brief but complete and coherent with the website content." v-model="previewStore.descrPreview"></textarea>
            <div class="h-[7px] bg-white/50 mt-3 rounded">
            ...
            <button :class="[previewStore.descrPreview === '' ? 'hidden' : '']" @click="copyDescription()" class="bg-white/50 rounded p-1 absolute bottom-9 right-2 w-[22px] h-[24px]">
        ...
        <button  :class="[previewStore.titlePreview === '' || previewStore.descrPreview === '' ? 'cursor-not-allowed bg-[#acacac]' : '']" 
        :disabled="previewStore.titlePreview === '' && previewStore.descrPreview === ''"
         @click="copyEmbed()" class=" bg-[#4692FE] p-2 rounded-xl text-white font-medium mt-8">
